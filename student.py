...
version = 6
basic_name = f'Unet_resnet_v{version}-1080-student'
...
y_last = np.array([(np.array(load_img("../input/test/masks/{}.png".format(idx), grayscale=True))) / 255 for idx in tqdm(test_df.index)]).reshape(-1, img_size_target, img_size_target, 1)

x_train = np.append(x_train, x_test, axis=0)
depth_train = np.append(depth_train, x_depth_test, axis=0)
y_train = np.append(y_train, y_last, axis=0)
